...
export function getObjName(
  obj: { constructor: { toString: () => any } } | null | undefined
) {
...
/**
 * Generate a 6-digit unique string with prefix `"__rnc_" + ${prefix}`
 */
export function getUniqueID(prefix: string = "") {
  return "__rnc_" + prefix + Math.random().toString(36).substring(2, 8);
}
...
export const STATE_BAR_HEIGHT = isIphoneX() ? 44 : 20; // 顶部状态栏高度
export const NAVIGATION_HEIGHT = 44; // 导航栏高度
export const SAFE_AREA_TOP_HEIGHT = isIphoneX() ? 88 : 64; // 安全区域 状态栏 + 导航栏高度
export const SAFE_AREA_BOTTOM_HEIGHT = isIphoneX() ? 49 + 34 : 49; // 安全区域 工具栏+ 底部距离
export const SAFE_BOTTOM_MARGIN = isIphoneX() ? 34 : 0;
