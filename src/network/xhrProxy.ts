...
import {
  ...
  RNConsoleRequestMethod,
} from "./requestItem";
import { IOnUpdateCallback } from "./helper";

export class XHRProxyHandler<T extends XMLHttpRequest>
  implements ProxyHandler<T>
{
  public XMLReq: XMLHttpRequest;
  public item: RNConsoleNetworkRequestItem;
  ...
  constructor(XMLReq: XMLHttpRequest, onUpdateCallback: IOnUpdateCallback) {
    ...
    this.item = new RNConsoleNetworkRequestItem();
  ...
  protected getOpen(target: T) {
    ...
    return (...args: Parameters<XMLHttpRequest["open"]>) => {
      ...
      this.item.method = method
        ? <RNConsoleRequestMethod>method.toUpperCase()
        : "GET";
      this.item.url = url || "";
      this.item.name =
  ...
  protected getSetRequestHeader(target: T) {
    ...
    return (...args: Parameters<XMLHttpRequest["setRequestHeader"]>) => {
  ...
  protected setOnReadyStateChange(
    target: T,
    key: string,
    value: XMLHttpRequest["onreadystatechange"]
  ) {
    return Reflect.set(
      target,
      key,
      (
        ...args: Parameters<Exclude<XMLHttpRequest["onreadystatechange"], null>>
      ) => {
        this.onReadyStateChange();
  ...
  protected setOnAbort(
    target: T,
    key: string,
    value: XMLHttpRequest["onabort"]
  ) {
    return Reflect.set(
      target,
      key,
      (...args: Parameters<Exclude<XMLHttpRequest["onabort"], null>>) => {
        this.onAbort();
        value?.apply(target, args);
      }
    );
  ...
  protected setOnTimeout(
    target: T,
    key: string,
    value: XMLHttpRequest["ontimeout"]
  ) {
    return Reflect.set(
      target,
      key,
      (...args: Parameters<Exclude<XMLHttpRequest["ontimeout"], null>>) => {
        this.onTimeout();
        value?.apply(target, args);
      }
    );
