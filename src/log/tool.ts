...
import { IRNConsoleLog, IRNConsoleLogData } from "./model";
...
export const getValueTextAndType = (val: any, wrapString = true) => {
  ...
  let text = val;
  if (val instanceof RNConsoleUninvocatableObject) {
...
export const isMatchedFilterText = (log: IRNConsoleLog, filterText: string) => {
  if (filterText === "") {
    return true;
  }
...
export const getLogDatasWithFormatting = (origDatas: any[]) => {
  ...
  if (tool.isString(origDatas[0]) && logFormattingPattern.test(origDatas[0])) {
    ...
    const logDataList: IRNConsoleLogData[] = [];
    ...
    while (mainLogs.length > 0) {
      ...
      if (isSetOrigData) {
        const log: IRNConsoleLogData = { origData };
        if (style) {
          log.style = style;
        }
    ...
    // (window as any)._rnOrigConsole.log('getLogDataWithSubstitutions format', logDataList);
    return logDataList;
  } else {
    const logDataList: IRNConsoleLogData[] = [];
    ...
    // (window as any)._rnOrigConsole.log('getLogDataWithSubstitutions normal', logDataList);
    return logDataList;
  }
};
 ...
 */
export class RNConsoleUninvocatableObject {}
